...
export default {
    ...
    */
    ...
    twelveMonthChart: async (req,res,next) => { // Gráfico estadístico de 12 meses
        // Nos retorna el total de venta de los ultimos 12 meses
        try{
            const reg = await models.Sale.aggregate(
                [
                    {   // como agruparé...
                        $group:{
                            _id:{  // agrupación de ventas por mes y año
    ...
    checkDates: async (req,res,next) => { // consulta de fechas
        try {
            ...
            const reg = await models.Sale.find({"createdAt" :{"$gte":start, "$lte":end}}) // filtra las fechas del modelo revenue por la fecha de comienzo y de fin que viajan desde el front
